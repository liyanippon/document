 ...
 */
public class FenyelistAction extends DispatchAction {
	 ...
	 */
	private static final Log log = LogFactory.getLog(FenyelistAction.class);
	...
	public ActionForward select(ActionMapping mapping, ActionForm form,
		...
		if (booleanUser) {
			...
			// 设置DAO和service的关系
			FenyeSelectInterface fenyeSelectInterface = getFenyeSelectInterface();
			...
			if (!boolean_next_previous) {
				try {
					log.fatal("首次进入分页状态");
					List<?> fenye = fenyeSelectInterface.FindSelectAll(
							StaticName.FIRST_RESULT, StaticName.MAX_RESULTS,
							StaticName.FENYESQL_SHUJU);
					// --------------------------------------------------------------------
					// 查询出分页需要的数据{默认数据库每页显示20条}
					Integer count = fenyeSelectInterface
							.FindSelectCount(StaticName.FENYESQL_COUNT);
					// 设置默认的上页下页
				...
				System.out
						.println("SQL访问时间测试结果:"
								...
								/ 300 + "秒");
				if (id.equals("dsf5413s130s32d1")) {
					return mapping.findForward("fenyeok");
				}
					return mapping.findForward("fenyeud");
				...
				try {
					...
					List fenye = fenyeSelectInterface
							.FindSelectAll(
									...
									StaticName.MAX_RESULTS,
									StaticName.FENYESQL_SHUJU);
					System.out.println("kkkkk:"+StaticName.FIRST_XIAYIYE);
					// next的SQL-count(*)的集合
					Integer count = fenyeSelectInterface
							.FindSelectCount(StaticName.FENYESQL_COUNT);
	...
	private FenyeSelectInterface getFenyeSelectInterface() {
