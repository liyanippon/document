	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ("search".equals(ftype)) {
				int toPage=Integer.parseInt(request.getParameter("toPage"));	//要显示的页面
				int start=(toPage-1)*17;	//显示某页数据的起始记录
				// 1.依据查询条件确定记录的总数
				...
				request.setAttribute("now", Long.valueOf(toPage));

				// 3.查询第一页的数据
				ArrayList list = enrollDAO.search(name, pid, specialtySubmit,
						eduSchool, classNo, joinState, Integer.valueOf(start),
						Integer.valueOf(17));
				request.setAttribute("enList", list);
				request.getRequestDispatcher("/jsp/backyard/index.jsp")
						.forward(request, response);
			}
			if("delete".equals(ftype)){
				String[] eid=request.getParameterValues("eid");	//勾选复选框的内容
				enrollDAO.delete(eid);
				request.getRequestDispatcher("/enroll/EnrollServlet?ftype=search&toPage=1").forward(request, response);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
